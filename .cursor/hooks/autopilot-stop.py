...
def _debug_log(payload: dict, continuing: bool) -> None:
    """Trace locale pour vérifier que Cursor a bien invoqué le hook."""
    log_path = ROOT / ".cursor" / "hooks" / "last-stop.log"
    try:
        log_path.write_text(
            json.dumps(
                {
...
def main() -> None:
    payload = _load_stdin()
    continuing = _should_continue(payload)
    _debug_log(payload, continuing)
    if continuing:
